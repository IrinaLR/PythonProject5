...
def sdb():
    ...
    if request.method == 'POST':
        var = str(request.form['var'])
    var2 = None
    var01 = None
    if var != 0:
        var1 = [list(i) for i in session.query(products).filter_by(ProductName=var)]
        var2 = int(var1[0][2])
        var01 = var1[0][1]
    else:
        var2 = ''
        var01 = ''
    return render_template('sdb.html', results=results, var2=var2, year=datetime.now().year, var01=var01)
...
def upd():
    if request.method == 'POST':
        ...
        session.query(products).filter_by(ProductName=var).update({"Price": price2}, synchronize_session='false')
        session.commit()
...
def tmn():
    ...
    ts = time.time()
    if var0 is not None:
        result_tmn = [list(i) for i in session.query(tm).filter_by(value=var0)]
    tf = time.time() - ts
    counter = 0
    for i in result_tmn:
        i = float(i[5].replace(',', '.'))
